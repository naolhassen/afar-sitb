...
export const NewsDetailPage: React.FC<NewsDetailPageProps> = ({
  ...
  return (
    <article className="mx-auto max-w-6xl px-4 pb-20 pt-32">
          ...
          <div className="sticky top-28 space-y-8">
            {relatedNews.length > 0 && (
              <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
                <h3 className="mb-4 text-lg font-bold text-zinc-900">Related News</h3>
                <div className="space-y-4">
                  {relatedNews.map((n) => (
                    <div
                      key={n.id}
                      onClick={() => router.get(`/${currentLocale}/news/${n.id}`)}
                      className="flex cursor-pointer items-start gap-3 rounded-xl p-2 transition hover:bg-zinc-50"
                    >
                      {n.image_url && (
                        <img
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold leading-snug text-zinc-800 hover:text-blue-600 line-clamp-2">
                          {tf(n, 'title', l)}
                        </p>
